...
impl<'a> TemporalDisc<'a> {
    pub fn time_march<T: SpatialDisc<D>, D: Dimension>(
        &mut self,
        spatial_disc: T,
        residuals: &mut Array<f64, Ix3>,
        solutions: &mut Array<f64, Ix3>,
    ) {
        while self.curr_step < self.solver_param.final_step
            && self.curr_time < self.solver_param.final_time
        {
