use ndarray::{
    Array1, Array2, ArrayView1, ArrayView2, ArrayView3, ArrayViewMut2, ArrayViewMut3,
    ArrayViewMut4, Axis, Zip, s,
};
...
pub trait SpaceTime1DSystem: Geometric2D {
    ...
    fn compute_residuals(
        ...
        let nequations = solutions.shape()[2];
        ...
        for (ielem, elem) in mesh.elements.iter().enumerate() {
            if let Status::Active(elem) = elem {
                ...
                let interp_sol = if is_enriched {
                    let mut result = Array2::zeros((ncell_basis, nequations));
                    for ieq in 0..nequations {
                        result.slice_mut(s![.., ieq]).assign(
                            &self
                                .interp_node_to_enriched_cubature()
                                .dot(&solutions.slice(s![ielem, .., ieq])),
                        );
                    }
                    result
                } else {
                    ...
                    for ieq in 0..nequations {
                        result.slice_mut(s![.., ieq]).assign(
                            &self.interp_node_to_cubature().dot(&solutions.slice(s![
                                ielem,
                                ..,
                                ieq
                            ])),
        ...
        for &iedge in mesh.interior_edges.iter() {
            if let Status::Active(edge) = &mesh.edges[iedge] {
                ...
                let (left_sol, right_sol) = {
                    let left_sol: Array2<f64> = solutions.slice(s![ilelem, .., ..]).select(
                        Axis(0),
                    ...
                    let right_sol: Array2<f64> = solutions.slice(s![irelem, .., ..]).select(
                        Axis(0),
                        sol_nodes_along_edges
                    ...
                    if is_enriched {
                        let mut left_interped_sol = Array2::zeros((ncell_basis, nequations));
                        for ieq in 0..nequations {
                            left_interped_sol.slice_mut(s![.., ieq]).assign(
                                &self
                                    .interp_node_to_enriched_quadrature()
                                    .dot(&left_sol.slice(s![.., ieq])),
                            );
                        }
                        let mut right_interped_sol = Array2::zeros((ncell_basis, nequations));
                        for ieq in 0..nequations {
                            right_interped_sol.slice_mut(s![.., ieq]).assign(
                                &self
                                    .interp_node_to_enriched_quadrature()
                                    .dot(&right_sol.slice(s![.., ieq])),
                            );
                        }
                        (left_interped_sol, right_interped_sol)
                    } else {
                        (left_sol, right_sol)
                    }
                };
                ...
                for i in 0..nedge_basis {
                    let left_value = left_sol.slice(s![i, ..]);
                    let right_value = right_sol.slice(s![nedge_basis - 1 - i, ..]);
                    ...
                    residuals
                        .slice_mut(s![ilelem, left_itest_func, ..])
                        .scaled_add(left_cano_length * edge_weights[i], &left_transformed_flux);
                    residuals
                        .slice_mut(s![irelem, right_itest_func, ..])
                        .scaled_add(right_cano_length * edge_weights[i], &right_transformed_flux);
                }
            }
        }
        // Constant boundaries
        for bnd in &mesh.boundaries.constant {
            ...
            for &iedge in iedges {
                if let Status::Active(edge) = &mesh.edges[iedge] {
                    ...
                    let sol = {
                        let sol = solutions.slice(s![ielem, .., ..]).select(
                            Axis(0),
                            sol_nodes_along_edges
                                .slice(s![local_edge_ids[0], ..])
                                .as_slice()
                        ...
                        if is_enriched {
                            let mut interped_sol = Array2::zeros((ncell_basis, nequations));
                            for ieq in 0..nequations {
                                interped_sol.slice_mut(s![.., ieq]).assign(
                                    &self
                                        .interp_node_to_enriched_quadrature()
                                        .dot(&sol.slice(s![.., ieq])),
                                );
                    ...
                    for i in 0..nedge_basis {
                        ...
                        residuals
                            .slice_mut(s![ielem, itest_func, ..])
                            .scaled_add(cano_length * edge_weights[i], &transformed_flux);
                    }
                }
            }
        }
    ...
    //     d_retval: f64,
    // ) -> (f64, f64);
    fn physical_flux(&self, u: &Array1<f64>) -> Array1<f64>;
