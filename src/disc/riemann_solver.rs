...
pub fn hllc1d(
    left_value: &Array<f64, Ix1>,
    right_value: &Array<f64, Ix1>,
    hcr: &f64,
) -> Result<[f64; 4], &'static str> {
    let mut flux = [0.0_f64; 3];
    let ql = [left_value[0], left_value[1], left_value[2]];
    let qr = [right_value[0], right_value[1], right_value[2]];
    let ul = ql[1] / ql[0];
    let ur = qr[1] / qr[0];
    ...
    if ql[0] < 0.0 || qr[0] < 0.0 {
        ...
        let p_star = {
            ...
            ((cl + cr - (zeta - 1.0) / 2.0 * (ur - ul)) / (cl / pl.powf(zeta) + cr / pr.powf(zeta)))
                .powf(1.0 / zeta)
        };
        let sl = {
            let ql_lower = {
        ...
        if sl >= 0.0 {
            ...
            let s_star = (pr - pl + ql[0] * ul * (sl - ul) - qr[0] * ur * (sr - ur))
                / (ql[0] * (sl - ul) - qr[0] * (sr - ur));
            if s_star >= 0.0 {
                let mut q_star = [0.0_f64; 4];
                q_star[0] = ql[0] * (sl - ul) / (sl - s_star);
                q_star[1] = ql[0] * (sl - ul) / (sl - s_star) * s_star;
                q_star[2] = ql[0] * (sl - ul) / (sl - s_star)
                    * (ql[2] / ql[0] + (s_star - ul) * (s_star + pl / (ql[0] * (sl - ul))));
                ...
                q_star[2] = qr[0] * (sr - ur) / (sr - s_star)
                    * (qr[2] / qr[0] + (s_star - ur) * (s_star + pr / (qr[0] * (sr - ur))));
